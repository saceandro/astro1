...
!
!
subroutine rk4(N,x,dx,yi,yf)
implicit none

real(8), intent(in) :: N,x,dx,yi(2)
...
! [1] compute eta_a
call right_hand_side(N,x,yi,Fnc) ! compute right hand side for (x,yi)
do ieq = 1,2
...
! [2] compute eta_b
call right_hand_side(N,xh,yh,Fnc) ! compute right hand side for (x,yi)
do ieq = 1,2
...
! [4] compute eta_b
call right_hand_side(N,x+dx,yh,Fnc) ! compute right hand side for (x,yi)
do ieq = 1,2
